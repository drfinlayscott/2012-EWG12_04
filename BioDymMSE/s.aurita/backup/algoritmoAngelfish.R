...
# scenarios #1 b0=0.2
scn <- rbind(
	expand.grid(Btrig=0.5, Ftar=0.75, maxHR=0.35, aLag=c(1,3,5), srvBias=c(0.5, 1, 1.5), cthBias=c(0.5,1), IEM="linear"),
	expand.grid(Btrig=0.3, Ftar=1, maxHR=c(0.35, 1), aLag=c(1,3,5), srvBias=c(0.5, 1, 1.5), cthBias=c(0.5,1), IEM="linear")
	)

MSEsimsb0.2unif <- list()
length(MSEsimsb0.2unif) <- nrow(scn)

for(i in 1:length(MSEsimsb0.2unif)){ 
	try(MSEsimsb0.2unif[[i]] <- mseBD(OM=OM, start=iniyr, sr=srBH, srRsdl=srRsdl, bounds=bounds, Btrig=scn[i,"Btrig"], Ftar=scn[i,"Ftar"], aLag=scn[i, "aLag"], srvBias=scn[i,"srvBias"], cthBias=scn[i,"cthBias"], IEM=scn[i,"IEM"]))
}

save(MSEsimsb0.2unif, file="RData.MSEsimsb0.2unif")
