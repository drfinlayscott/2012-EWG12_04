mseBDunif <- function (OM, start, sr, srRsdl = FLQuant(1, dimnames = dimnames(window(rec(OM),
	...
	if(iYr %in% aYrs){
		...
		# abundance index bias
		#index(bd)[,dtaYrs] <- index(bd)[,dtaYrs]*srvBias
		...
		#catch(bd)[,dtaYrs] <- catch(bd)[,dtaYrs]*cthBias
...
mseBDnounif <- function (OM, start, sr, srRsdl = FLQuant(1, dimnames = dimnames(window(rec(OM),
    start = start))), CV = 0.2, Ftar = 0.75, Btrig = 0.75, Fmin = Ftar * 0.1, 
    Blim = Btrig * 1e-04, bounds = NULL, aLag=1, maxHR=0.35, maxF=2, IEM="linear", 
    cthBias=1, srvBias=1, seed=1234, amod="bd", clag=5, slag=5){
